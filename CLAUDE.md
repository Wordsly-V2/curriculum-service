...
| `messaging/` | Kafka producer only. `PATH_ITEMS_RETIRED_TOPIC` (`{ itemIds }`, ≤500 per message) tells learning-service to drop progress for items a publish drops: those in the previously active release and not in the new one (`retiredItemIds`), sent after the commit; a failed send is logged, not thrown. `activate` never retires, so rolling forward again finds the cards intact. It no-ops without `KAFKA_BROKERS`. |
