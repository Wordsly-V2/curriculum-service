...
| `admin-path/` | Authoring API under `/admin/path`, `@Roles('admin')` at class level (non-admins get 403). Reads: `overview` (the whole working copy, archived rows included, as a tree; every node carries `status` and `origin`: `seed` = untouched since the last import, `edited` = admin-changed seeded row, `admin` = no seed; pure in `admin-path.logic.ts`), `validate` (the publish checks, nothing written), `seed-plan` (an importer dry run: what `content/` would insert/update and where it conflicts with admin edits), `releases` (newest first, active marked). |
